...
export class MinioService {
	...
	async uploadPhoto(value: string): Promise<string> {
		const name = uuid()

		const image = Buffer.from(
			value.replace(/^data:image\/\w+;base64,/i, ''),
			'base64',
		)
		...
		await this.#_client.putObject(
			this.#_bucket,
			`story/${name}.${metadata.format}`,
			image,
		)

		return `${this.#_bucket}/story/${name}.${metadata.format}`
	}

	async deletePhoto(value: string): Promise<void> {
		await this.#_client.removeObject(
			this.#_bucket,
			value.replace(`${this.#_bucket}/`, ''),
		)
	}
}
